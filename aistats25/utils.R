...
#MOSEKLM_LICENSE_FILE= "/Users/.../mosek/mosek.lic"
#Sys.getenv("MOSEKLM_LICENSE_FILE")
#Sys.setenv(MOSEKLM_LICENSE_FILE = "/Users/.../mosek/mosek.lic")
...
fr_cate <- function(dat, 
                    ...
                    nuisance.est='rf',
                    ps.trim = "Sturmer.1",
                    delta=delta, sqr=TRUE, interactions=TRUE) {
  ...
  for (k in 1:2) {
    ...
    if (nuisance.est=="rf") {
      ...
      pi.rf <- ranger(f.pi, dat=dat.train, write.forest = TRUE, verbose = FALSE,seed = 123) 
      mu.rf <- ranger(f.mu, dat=dat.train, write.forest = TRUE, verbose = FALSE,seed = 123) 
  ...
  epsilon <- 1e-4  # 根据实际需要调整这个值 ##holly
  Dmat_eps <- Q.mat + diag(epsilon, nrow(Q.mat)) ##holly
  C.mat <- NULL
  #if (solver=="quadprog") {
    ...
    #model <- solve.QP(Dmat=Dmat_eps,dvec=phi.b.hat,Amat=t(C.mat),bvec=d.vec) #holly
    #beta.hat <- model$solution
  #}
  if (solver == "quadprog") {
    for (s.nm in nm.s.features) {
      d_j <- ((1 - dat[, s.nm]) %*% b.mat / (n * mean(1 - dat[, s.nm]))) - 
        (dat[, s.nm] %*% b.mat / (n * mean(dat[, s.nm])))
      temp.C <- rbind(
        -d_j,  # -d_j^T * beta >= -delta_j
        d_j    # d_j^T * beta >= -delta_j
      )
      C.mat <- rbind(C.mat, temp.C)
    }
    ...
    model <- solve.QP(Dmat = Dmat_eps, dvec = phi.b.hat, Amat = C.mat, bvec = d.vec)
    beta.hat <- model$solution
  }
  ...
  if (solver=="mosek") {
    ...
    prob$c <- as.vector(prob$c)  ##add by holly
    ...
    prob$bc <- cbind(
      blc = rep(-delta, nrow(C.mat)),  # 下界
      buc = rep(delta, nrow(C.mat)))    # 上界
